...
def observability_job_trace(job_id: str, db: Session = Depends(get_db)) -> dict:
    ...
    return _ok({
        "job": _serialize(job, ("job_id", "message_id", "target_agent_id", "target_queue", "status", "retry_count", "max_retries", "latest_run_id", "result_artifact_id", "output_contract_json", "conversation_id", "created_at", "updated_at")),
...
def observability_triage(db: Session = Depends(get_db)) -> dict:
    ...
    failure_items = [_serialize(job, ("job_id", "target_agent_id", "target_queue", "status", "retry_count", "latest_run_id", "output_contract_json", "conversation_id", "updated_at")) for job in recent_failures]
