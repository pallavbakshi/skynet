...
def observability_triage(db: Session = Depends(get_db)) -> dict:
    ...
    failure_items = [_serialize(job, ("job_id", "target_agent_id", "target_queue", "status", "retry_count", "latest_run_id", "output_contract_json", "conversation_id", "timeout_seconds", "deadline_at", "updated_at")) for job in recent_failures]
