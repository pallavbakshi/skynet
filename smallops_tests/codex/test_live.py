"""Env-gated live canaries for the smallops Codex driver."""
...
from smallops import Config, IdleReason, Session
...
@pytest.mark.live
def test_live_codex_exact_reply(
    request: pytest.FixtureRequest,
    tmp_path: Path,
    smallops_mux: str,
) -> None:
    spec = Spec(
        prompt="Reply with exactly: CODEX-PONG",
        oracle=Contains("CODEX-PONG"),
        tui="codex",
        mux=smallops_mux,
        timeout=120.0,
    )
...
@pytest.mark.live
def test_live_codex_file_read_tool_use(
    request: pytest.FixtureRequest,
    tmp_path: Path,
    smallops_mux: str,
) -> None:
    (tmp_path / "README.md").write_text("SKYNET-CODEX-LIVE-CANARY\n\nBody text.\n", encoding="utf-8")
    spec = Spec(
        prompt="Read README.md and reply with exactly its first line.",
        oracle=AllOf((Invariant(assert_tool_use), Contains("SKYNET-CODEX-LIVE-CANARY"))),
        tui="codex",
        mux=smallops_mux,
        timeout=180.0,
    )
...
@pytest.mark.live
def test_live_codex_nudge_then_wait(
    request: pytest.FixtureRequest,
    tmp_path: Path,
    smallops_mux: str,
) -> None:
